 ...
 *
 * Usage: wp eval-file restore-option.php <backup-file> [preview]
 *
...
$cr_same    = $cr_exists ? ( null !== $cr_current && $cr_enc( $cr_current ) === $cr_enc( $cr_value ) ) : null === $cr_current;
$cr_keys    = array();
if ( is_array( $cr_current ) && is_array( $cr_value ) ) {
	foreach ( array_unique( array_merge( array_keys( $cr_current ), array_keys( $cr_value ) ) ) as $cr_k ) {
		if ( $cr_enc( $cr_current[ $cr_k ] ?? null ) !== $cr_enc( $cr_value[ $cr_k ] ?? null ) ) {
			$cr_keys[] = (string) $cr_k;
		}
	}
}
$cr_report = array( 'home' => home_url( '/' ), 'option' => $cr_name, 'backup' => basename( $cr_file ), 'identical' => $cr_same, 'differing_keys' => $cr_keys, 'preview' => $cr_preview );
if ( $cr_same ) {
...
$cr_ok    = $cr_exists ? ( null !== $cr_after && $cr_enc( $cr_after ) === $cr_enc( $cr_value ) ) : null === $cr_after;
echo "\nCRJSON:" . wp_json_encode( $cr_report + array( 'restored' => $cr_ok, 'before_restore_backup' => basename( $cr_before ) ), JSON_UNESCAPED_SLASHES ) . "\n";
if ( ! $cr_ok ) {
	WP_CLI::error( sprintf( 'Option %s did not store the backup value; the pre-restore value is in %s.', $cr_name, basename( $cr_before ) ) );
}

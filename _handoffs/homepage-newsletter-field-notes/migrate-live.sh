...
# Run from your machine (uses the wp @live alias). Each step prints its dry-run
...
$W post get $MENU_ID --field=post_content > "$VAULT/release/menu-$MENU_ID-live-before.html"
grep -q 'Browse all kinds' "$VAULT/release/menu-$MENU_ID-live-before.html" && echo "menu already regrouped" || $W post update $MENU_ID - < "$VAULT/nav/nav-proposed.html"

echo "## 6. Stream page: Browse all"
$W post get $STREAM_ID --field=post_content > "$VAULT/release/stream-$STREAM_ID-live-before.html"
if grep -q 'cr-browse-all' "$VAULT/release/stream-$STREAM_ID-live-before.html"; then echo "already present"; else { cat "$VAULT/release/stream-$STREAM_ID-live-before.html"; printf '\n\n<!-- wp:pattern {"slug":"courtneyr-child/cr-browse-all"} /-->\n'; } | $W post update $STREAM_ID -; fi

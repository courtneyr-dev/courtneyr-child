...
function add_title_lede( string $html, array $block ): string {
	...
	if ( $rating > 0 ) {
		$star  = '<svg viewBox="0 0 24 24" fill="currentColor" focusable="false"><path d="M12 2l3 6.5 7 .6-5.3 4.6 1.6 6.8L12 17l-6.9 3.5 1.6-6.8L1.4 9.1l7-.6z"/></svg>';
		...
		$lede .= '<span class="cr-journal__rating-value">' . esc_html( sprintf( '%d / 5', $rating ) ) . '</span></p>';
	}
...
function vhs_mechanics( bool $heavy_left = true ): string {
	...
	return '<span class="cr-vhs__reel cr-vhs__reel--l cr-vhs__reel--' . $l . '"></span>'
		. '<span class="cr-vhs__reel cr-vhs__reel--r cr-vhs__reel--' . $r . '"></span>'
		. '<span class="cr-vhs__tape"></span>';
}
...
function vhs_label( array $facts = array() ): string {
	...
	$out .= '<span class="cr-vhs__note cr-hand" aria-hidden="true">' . esc_html__( 'Be kind & rewind', 'courtneyr-child' ) . '</span>';
	$out .= '<span class="cr-vhs__stripe"></span>';
	if ( ! empty( $facts ) ) {
